...
public final class FirstPartTasks {
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(
                Comparator.comparing(
                        album -> album.getTracks().stream().max(Comparator.comparing(Track::getRating)).get().getRating()
                )
        );
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums
                .sorted(Comparator.comparing(album -> -album.getTracks().stream().mapToInt(Track::getRating).average().getAsDouble()))
                .collect(Collectors.toList());
    }
